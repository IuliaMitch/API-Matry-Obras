...
const findAllObrasService = () => {
  return Obras;
};

const findByIdObraService = (paramsId) => {
  return Obras.find((obra) => obra.id == paramsId);
};

const createObraService = (newObra) => {
  const newId = Obras.length + 1;
  newObra.id = newId;
  Obras.push(newObra);
  return newObra;
};

const updateByIdObraService = (id, obraEdited) => {
  obraEdited["id"] = id;
  const obraIndex = Obras.findIndex((obra) => obra.id == id);
  Obras[obraIndex] = obraEdited;
  return obraEdited;
};

const deleteByIdObraService = (id) => {
    const obraIndex = Obras.findIndex((obra) => obra.id == id);
    return Obras.splice(obraIndex, 1);
};
